...
function influactive_form_edit( string $hook ): void {
	...
	wp_enqueue_style( 'influactive-form', plugin_dir_url( __FILE__ ) . 'back-end/post-type/form/form.min.css', array(), '1.2.4' );

	wp_enqueue_script( 'influactive-tabs', plugin_dir_url( __FILE__ ) . 'dist/backEndTab.bundled.js', array(), '1.2.4', true );
	wp_enqueue_style( 'influactive-tabs', plugin_dir_url( __FILE__ ) . 'back-end/post-type/tab/tab.min.css', array(), '1.2.4' );

	wp_enqueue_style( 'influactive-form-layout', plugin_dir_url( __FILE__ ) . 'back-end/post-type/layout/layout.min.css', array(), '1.2.4' );
	wp_enqueue_script( 'influactive-form-layout', plugin_dir_url( __FILE__ ) . 'dist/backEndLayout.bundled.js', array(), '1.2.4', true );
	...
	wp_enqueue_style( 'influactive-form-style', plugin_dir_url( __FILE__ ) . 'back-end/post-type/style.min.css', array(), '1.2.4' );

	wp_enqueue_style( 'influactive-form-preview', plugin_dir_url( __FILE__ ) . 'front-end/form.min.css', array(), '1.2.4' );

	$form_id = get_post_meta( get_the_ID(), 'influactive_form_id', true );
	if ( ! $form_id ) {
		throw new RuntimeException( "Form ID not found. Exiting..." );
	}
	wp_enqueue_style( 'influactive-form-dynamic-style', plugin_dir_url( __FILE__ ) . 'front-end/dynamic-style.php?post_id=' . $form_id, array(), '1.2.4' );
}
...
function influactive_form_shortcode_enqueue(): void {
	...
	wp_enqueue_script( 'influactive-form', plugin_dir_url( __FILE__ ) . 'dist/frontEnd.bundled.js', $script_handle, '1.2.4', true );
	wp_enqueue_style( 'influactive-form', plugin_dir_url( __FILE__ ) . 'front-end/form.min.css', [], '1.2.4' );
