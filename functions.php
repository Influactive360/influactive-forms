...
function influactive_form_edit( string $hook ): void {
	...
	wp_enqueue_style( 'influactive-form-style', plugin_dir_url( __FILE__ ) . 'back-end/post-type/style.min.css', array(), '1.2.2' );

	wp_enqueue_style( 'influactive-form-preview', plugin_dir_url( __FILE__ ) . 'front-end/form.min.css', array(), '1.2.2' );

	$form_id = get_post_meta( get_the_ID(), 'influactive_form_id', true );
	if ( ! $form_id ) {
		throw new RuntimeException( "Form ID not found. Exiting..." );
	}
	wp_enqueue_style( 'influactive-form-dynamic-style', plugin_dir_url( __FILE__ ) . 'front-end/dynamic-style.php?post_id=' . $form_id, array(), '1.2.2' );
}
